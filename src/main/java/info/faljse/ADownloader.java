...
public class ADownloader {
    ...
    private void dlStreamItem(Broadcast broadcastDetail) {
        HttpClient client = HttpClient.newHttpClient();
        String urlt=broadcastDetail.getStreams().getFirst().getUrls().getProgressive();
        ...
        String fileName= String.format("%s_%d.mp3", dateStr, broadcastDetail.getId());
        ...
        try (OutputStream jsonOut = new FileOutputStream(Paths.get(folderName, fileName+".json").toFile())) {
            ObjectMapper objectMapper = new ObjectMapper();
        ...
        try (OutputStream outStream = new FileOutputStream(Paths.get(folderName, fileName + ".part").toFile())) {
            ...
            while ((bytesRead = response.body().read(buffer)) != -1) {
                ...
                if (totalBytesRead - lastBytesRead > 1024 * 1024) {
                    // System.out.printf("%d MB%n", totalBytesRead/(1024*1024));
            ...
            File fileToMove = Paths.get(folderName, fileName + ".part").toFile();
